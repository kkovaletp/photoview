...
import EyeOffIcon from './icons/eyeOffIcon.svg?react'
          ...
          <div className="absolute top-1/2 right-0 -translate-y-1/2 flex items-center">
            {isPassword && (
              <button
                ...
                aria-label={revealed
                  ? t('general.action.hide_pass', 'Hide password')
                  : t('general.action.show_pass', 'Show password')
                }
            ...
            {loading ? (
              <LoadingSpinnerIcon
                aria-label={t('general.action.loading', 'Loading')}
                className={clsx('mr-2 animate-spin', iconClassName)}
              />
            ) : (
              action && (
                <button
                ...
                >
                  <ActionArrowIcon className={clsx(
                    sizeVariant === 'big' && 'w-4 h-4 mt-1 mr-1'
                  )}
                  />
                </button>
