...
LIBRAW_VERSION=$(curl -s ${GITHUB_TOKEN:+-H "Authorization: Bearer ${GITHUB_TOKEN}"} \
  https://api.github.com/repos/LibRaw/LibRaw/releases/latest | jq -r '.tag_name')
LIBHEIF_VERSION=$(curl -s ${GITHUB_TOKEN:+-H "Authorization: Bearer ${GITHUB_TOKEN}"} \
  https://api.github.com/repos/strukturag/libheif/releases/latest | jq -r '.tag_name')
IMAGEMAGICK_VERSION=$(curl -s ${GITHUB_TOKEN:+-H "Authorization: Bearer ${GITHUB_TOKEN}"} \
  https://api.github.com/repos/ImageMagick/ImageMagick/releases/latest | jq -r '.tag_name')
JELLYFIN_FFMPEG_VERSION=$(curl -s ${GITHUB_TOKEN:+-H "Authorization: Bearer ${GITHUB_TOKEN}"} \
